...
var PathProbe = require("path-probe.js")
...
function configure(installDirectory, moduleName, outputBaseDir, jsonProbe) {
    if (!installDirectory || installDirectory.trim() === "") {
        throw "Conan install directory is empty. Set moduleProviders.conan.installDirectory in your product.";
    }

    if (!File.exists(FileInfo.joinPaths(installDirectory, "conan-qbs-deps"))) {
        throw "Direcotry conan-qbs-deps doesn't exist. ";
    }
    ...
    const moduleInfo = JSON.parse(moduleFileInfo.readAll());
    ...
    writeCppProperty("systemIncludePaths", cppInfo.includedirs.map(FileInfo.fromNativeSeparators));
    writeCppProperty("libraryPaths", cppInfo.libdirs.map(FileInfo.fromNativeSeparators));
