...
//---------------------------------------------------------------------------------------------------------------------
TST_QTextCodec::TST_QTextCodec(QObject *parent)
  : AbstractTest{parent}
{
}
...
void TST_QTextCodec::toUnicode_data()
{
    ...
    QTest::newRow("korean-eucKR") << QFINDTESTDATA("codecs/korean.txt") << "eucKR";
#endif
    QTest::newRow("UTF-8") << QFINDTESTDATA("codecs/utf8.txt") << "UTF-8";
}
...
void TST_QTextCodec::toUnicode()
{
    ...
    if (file.open(QIODevice::ReadOnly))
    {
        ...
        QTextEncoder *encoderWithoutBom = c->makeEncoder(QTextCodec::IgnoreHeader);
        QCOMPARE(ba, encoderWithoutBom->fromUnicode(uniString));
        QCOMPARE(ba, encoderWithoutBom->fromUnicode(QStringView(uniString)));
...
#endif // WITH_ICU_CODECS || WITH_BIG_CODECS
...
void TST_QTextCodec::fromUnicode()
{
    ...
    if (eightBit)
    {
        char chars[128];
        for (int i = 0; i < 128; ++i)
            chars[i] = static_cast<char>(i + 128);
...
{
    QFile file(QFINDTESTDATA("codecs/QT4-crashtest.txt"));
...
void TST_QTextCodec::codecForLocale()
{
    ...
    QByteArray localeEncodedTimeString = encoderWithoutBom->fromUnicode(unicodeTimeString);
...
void TST_QTextCodec::asciiToIscii() const
{
    ...
    static const char *const isciiCodecs[] = {"Iscii-Mlm",
                                              "Iscii-Knd",
                                              "Iscii-Tlg",
                                              "Iscii-Tml",
                                              "Iscii-Ori",
    ...
    for (int i = 0; i < isciiCodecsLen; ++i)
    {
        ...
        for (int i2 = 0; i2 < len; ++i2)
        {
            ...
            QVERIFY2(textCodec->canEncode(c),
                     qPrintable(
                         QString::fromLatin1("Failed to encode %1 with encoding %2")
                             .arg(QString::number(c.unicode()), QString::fromLatin1(textCodec->name().constData()))));
        }

        QVERIFY2(textCodec->canEncode(ascii),
                 qPrintable(QString::fromLatin1("Failed for full string with encoding %1")
...
void TST_QTextCodec::unicodeToISCII()
{
    ...
    QCOMPARE(codec->fromUnicode(steel.constData(), static_cast<int>(steel.size()), &state), ba);
    state.clear();
    QCOMPARE(codec->toUnicode(ba.constData(), static_cast<int>(ba.size()), &state), steel);
    ...
    state.clear();
    QCOMPARE(codec->fromUnicode(zwnj.constData(), static_cast<int>(zwnj.size()), &state), ba);
    state.clear();
    QCOMPARE(codec->toUnicode(ba.constData(), static_cast<int>(ba.size()), &state), zwnj);

    ba = QByteArray::fromHex("e8e9");
    auto zwj = QString("\u094d\u200d");

    state.clear();
    QCOMPARE(codec->fromUnicode(zwj.constData(), static_cast<int>(zwj.size()), &state), ba);
    state.clear();
    QCOMPARE(codec->toUnicode(ba.constData(), static_cast<int>(ba.size()), &state), zwj);
#else
    QSKIP("Not available");
#endif
}
...
void TST_QTextCodec::nonFlaggedCodepointFFFF() const
{
    ...
    QTextEncoder *encoderWithoutBom = codec->makeEncoder(QTextCodec::IgnoreHeader);
    const QByteArray asDecoded = encoderWithoutBom->fromUnicode(QStringView(&ch, 1));
    QCOMPARE(asDecoded, QByteArray("\357\277\277"));

    QByteArray ffff("\357\277\277");
    QTextCodec::ConverterState state(QTextCodec::ConvertInvalidToNull);
    QVERIFY(codec->toUnicode(ffff.constData(), static_cast<int>(ffff.size()), &state) == QByteArray::fromHex("EFBFBF"));
}
...
void TST_QTextCodec::flagF7808080() const
{
    ...
    QTextCodec::ConverterState state(QTextCodec::ConvertInvalidToNull);
    QCOMPARE(codec->toUnicode(input.constData(), static_cast<int>(input.size()), &state),
             QString(input.size(), QChar(0)));
}
...
void TST_QTextCodec::nonFlaggedEFBFBF() const
{
...

    {
        ...
        QTextCodec::ConverterState state(QTextCodec::ConvertInvalidToNull);
        QVERIFY(codec->toUnicode(validInput.constData(), static_cast<int>(validInput.size()), &state)
                == QByteArray::fromHex("EFBFBF"));
    ...
    // Check that 0xEFBFBF is correctly decoded when preceded by an arbitrary character
    {
        ...
        QTextCodec::ConverterState state(QTextCodec::ConvertInvalidToNull);
        QVERIFY(codec->toUnicode(start.constData(), static_cast<int>(start.size()), &state)
                == QByteArray("B").append(QByteArray::fromHex("EFBFBF")));
    }
}
...
void TST_QTextCodec::asciiToTSCII()
{
    ...
    for (int i2 = 0; i2 < len; ++i2)
    {
        ...
        QVERIFY2(textCodec->canEncode(c),
                 qPrintable(
                     QString::fromLatin1("Failed to encode %1 with encoding TSCII").arg(QString::number(c.unicode()))));
    }
...
void TST_QTextCodec::unicodeToTSCII()
{
    ...
    QTextCodec::ConverterState state;
    QCOMPARE(codec->fromUnicode(st.constData(), static_cast<int>(st.size()), &state), ba);
    QCOMPARE(codec->toUnicode(ba.constData(), static_cast<int>(ba.size()), &state), st);
...
void TST_QTextCodec::utf32Codec_data()
{
    ...
    struct UtfTestRow
    {
        QVector<char32_t> utf32;
        QString utf16;
    };

    auto makeUtfTestRow = [](int n) -> UtfTestRow
    {
        const uint zeroVal = 0x11136; // Unicode's representation of Chakma zero
        const char32_t digit = zeroVal + static_cast<uint>(n);
        const QVector<char32_t> u32 = {digit};
        const QChar u16[] = {QChar::highSurrogate(digit), QChar::lowSurrogate(digit)};
        return {u32, QString(u16, 2)};
    };

    for (int n = 0; n < 10; ++n)
    {
        auto const row = makeUtfTestRow(n);
        QTest::addRow("Chakma digit %d", n) << row.utf32 << row.utf16;
    }
}
...
void TST_QTextCodec::utf32Codec()
{
    ...
    QByteArray encoded(reinterpret_cast<const char *>(utf32.data()),
                       static_cast<vsizetype>(sizeof(char32_t) * static_cast<unsigned long long>(utf32.size())));
...
void TST_QTextCodec::utf8Codec_data()
{
    ...
    QTest::newRow("str1") << QByteArray(
        "\303\266\303\244\303\274\303\226\303\204\303\234\303\270\303\246\303\245\303\230\303\206\303\205")
                          << QString::fromLatin1("\366\344\374\326\304\334\370\346\345\330\306\305") << -1;
    QTest::newRow("str1-len") << QByteArray(
        "\303\266\303\244\303\274\303\226\303\204\303\234\303\270\303\246\303\245\303\230\303\206\303\205")
    ...
    str += QChar(0xd800);
    str += QChar(0xdf04);
    QTest::newRow("surrogate") << QByteArray(
        "Old Italic: \360\220\214\200\360\220\214\201\360\220\214\202\360\220\214\203\360\220\214\204")
                               << str << -1;

    QTest::newRow("surrogate-len") << QByteArray(
        "Old Italic: \360\220\214\200\360\220\214\201\360\220\214\202\360\220\214\203\360\220\214\204")
                                   << str.left(16) << 20;
...
void TST_QTextCodec::utf8Codec()
{
    ...
    QString str = codec->toUnicode(utf8.isNull() ? 0 : utf8.constData(), len < 0 ? int(qstrlen(utf8.constData())) : len);
    QCOMPARE(str, res);
...
void TST_QTextCodec::utf8bom_data()
{
    ...
    QTest::newRow("nobom") << QByteArray("\302\240", 2) << QString::fromLatin1("\240");

    {
        static const char16_t data[] = {0x201d};
        QTest::newRow("nobom 2") << QByteArray("\342\200\235", 3) << QString::fromUtf16(data, std::size(data));
    }

    {
        static const char16_t data[] = {0xf000};
        QTest::newRow("bom1") << QByteArray("\357\200\200", 3) << QString::fromUtf16(data, std::size(data));
    }

    {
        static const char16_t data[] = {0xfec0};
        QTest::newRow("bom2") << QByteArray("\357\273\200", 3) << QString::fromUtf16(data, std::size(data));
    }

    {
        QTest::newRow("normal-bom") << QByteArray("\357\273\277a", 4) << QString("a");
    }

    { // test the non-SIMD code-path
        static const char16_t data[] = {0x61, 0xfeff, 0x62};
        QTest::newRow("middle-bom (non SIMD)")
            << QByteArray("a\357\273\277b") << QString::fromUtf16(data, std::size(data));
    }

    { // test the SIMD code-path
        static const char16_t data[]
            = {0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69, 0x6a, 0x6b, 0x6c, 0xfeff, 0x6d};
        QTest::newRow("middle-bom (SIMD)")
            << QByteArray("abcdefghijkl\357\273\277m") << QString::fromUtf16(data, std::size(data));
    }
}
...
void TST_QTextCodec::utf8bom()
{
    ...
    QCOMPARE(codec->toUnicode(data.constData(), static_cast<int>(data.size()), 0), result);

    QTextCodec::ConverterState state;
    QCOMPARE(codec->toUnicode(data.constData(), static_cast<int>(data.size()), &state), result);
}
...
void TST_QTextCodec::utf8stateful()
{
    ...
    QString decoded1 = utf8codec->toUnicode(buffer1, static_cast<int>(buffer1.size()), &state);
    if (result.isNull())
    {
    ...
    QString decoded2 = utf8codec->toUnicode(buffer2, static_cast<int>(buffer2.size()), &state);
...
void TST_QTextCodec::utfHeaders_data()
{
    ...
    QTest::newRow("utf8 bom") << QByteArray("UTF-8") << static_cast<int>(QTextCodec::DefaultConversion)
                              << QByteArray("\xef\xbb\xbfhello") << QString::fromLatin1("hello") << true;
    QTest::newRow("utf8 nobom") << QByteArray("UTF-8") << static_cast<int>(QTextCodec::DefaultConversion)
                                << QByteArray("hello") << QString::fromLatin1("hello") << true;
    QTest::newRow("utf8 bom ignore header")
        << QByteArray("UTF-8") << static_cast<int>(QTextCodec::IgnoreHeader) << QByteArray("\xef\xbb\xbfhello")
        << (QString(QChar(0xfeff)) + QString::fromLatin1("hello")) << true;
    QTest::newRow("utf8 nobom ignore header") << QByteArray("UTF-8") << static_cast<int>(QTextCodec::IgnoreHeader)
                                              << QByteArray("hello") << QString::fromLatin1("hello") << true;

    QTest::newRow("utf16 bom be") << QByteArray("UTF-16") << static_cast<int>(QTextCodec::DefaultConversion)
                                  << QByteArray("\xfe\xff\0h\0e\0l", 8) << QString::fromLatin1("hel") << true;
    QTest::newRow("utf16 bom le") << QByteArray("UTF-16") << static_cast<int>(QTextCodec::DefaultConversion)
                                  << QByteArray("\xff\xfeh\0e\0l\0", 8) << QString::fromLatin1("hel") << true;
    if (QSysInfo::ByteOrder == QSysInfo::BigEndian)
    {
        QTest::newRow("utf16 nobom") << QByteArray("UTF-16") << static_cast<int>(QTextCodec::DefaultConversion)
                                     << QByteArray("\0h\0e\0l", 6) << QString::fromLatin1("hel") << true;
        QTest::newRow("utf16 bom be ignore header")
            << QByteArray("UTF-16") << static_cast<int>(QTextCodec::IgnoreHeader) << QByteArray("\xfe\xff\0h\0e\0l", 8)
            << (QString(QChar(0xfeff)) + QString::fromLatin1("hel")) << true;
    }
    else
    {
        QTest::newRow("utf16 nobom") << QByteArray("UTF-16") << static_cast<int>(QTextCodec::DefaultConversion)
                                     << QByteArray("h\0e\0l\0", 6) << QString::fromLatin1("hel") << true;
        QTest::newRow("utf16 bom le ignore header")
            << QByteArray("UTF-16") << static_cast<int>(QTextCodec::IgnoreHeader) << QByteArray("\xff\xfeh\0e\0l\0", 8)
            << (QString(QChar(0xfeff)) + QString::fromLatin1("hel")) << true;
    }

    QTest::newRow("utf16-be bom be") << QByteArray("UTF-16BE") << static_cast<int>(QTextCodec::DefaultConversion)
                                     << QByteArray("\xfe\xff\0h\0e\0l", 8) << QString::fromLatin1("hel") << true;
    QTest::newRow("utf16-be nobom") << QByteArray("UTF-16BE") << static_cast<int>(QTextCodec::DefaultConversion)
                                    << QByteArray("\0h\0e\0l", 6) << QString::fromLatin1("hel") << true;
    QTest::newRow("utf16-be bom be ignore header")
        << QByteArray("UTF-16BE") << static_cast<int>(QTextCodec::IgnoreHeader) << QByteArray("\xfe\xff\0h\0e\0l", 8)
        << (QString(QChar(0xfeff)) + QString::fromLatin1("hel")) << true;

    QTest::newRow("utf16-le bom le") << QByteArray("UTF-16LE") << static_cast<int>(QTextCodec::DefaultConversion)
                                     << QByteArray("\xff\xfeh\0e\0l\0", 8) << QString::fromLatin1("hel") << true;
    QTest::newRow("utf16-le nobom") << QByteArray("UTF-16LE") << static_cast<int>(QTextCodec::DefaultConversion)
                                    << QByteArray("h\0e\0l\0", 6) << QString::fromLatin1("hel") << true;
    QTest::newRow("utf16-le bom le ignore header")
        << QByteArray("UTF-16LE") << static_cast<int>(QTextCodec::IgnoreHeader) << QByteArray("\xff\xfeh\0e\0l\0", 8)
        << (QString(QChar(0xfeff)) + QString::fromLatin1("hel")) << true;

    QTest::newRow("utf32 bom be") << QByteArray("UTF-32") << static_cast<int>(QTextCodec::DefaultConversion)
                                  << QByteArray("\0\0\xfe\xff\0\0\0h\0\0\0e\0\0\0l", 16) << QString::fromLatin1("hel")
                                  << true;
    QTest::newRow("utf32 bom le") << QByteArray("UTF-32") << static_cast<int>(QTextCodec::DefaultConversion)
                                  << QByteArray("\xff\xfe\0\0h\0\0\0e\0\0\0l\0\0\0", 16) << QString::fromLatin1("hel")
                                  << true;
    if (QSysInfo::ByteOrder == QSysInfo::BigEndian)
    {
        QTest::newRow("utf32 nobom") << QByteArray("UTF-32") << static_cast<int>(QTextCodec::DefaultConversion)
                                     << QByteArray("\0\0\0h\0\0\0e\0\0\0l", 12) << QString::fromLatin1("hel") << true;
        QTest::newRow("utf32 bom be ignore header")
            << QByteArray("UTF-32") << static_cast<int>(QTextCodec::IgnoreHeader)
    ...
    {
        QTest::newRow("utf32 nobom") << QByteArray("UTF-32") << static_cast<int>(QTextCodec::DefaultConversion)
                                     << QByteArray("h\0\0\0e\0\0\0l\0\0\0", 12) << QString::fromLatin1("hel") << true;
    ...
    QTest::newRow("utf32-be bom be") << QByteArray("UTF-32BE") << static_cast<int>(QTextCodec::DefaultConversion)
                                     << QByteArray("\0\0\xfe\xff\0\0\0h\0\0\0e\0\0\0l", 16)
                                     << QString::fromLatin1("hel") << true;
    QTest::newRow("utf32-be nobom") << QByteArray("UTF-32BE") << static_cast<int>(QTextCodec::DefaultConversion)
                                    << QByteArray("\0\0\0h\0\0\0e\0\0\0l", 12) << QString::fromLatin1("hel") << true;
    QTest::newRow("utf32-be bom be ignore header")
        << QByteArray("UTF-32BE") << static_cast<int>(QTextCodec::IgnoreHeader)
        << QByteArray("\0\0\xfe\xff\0\0\0h\0\0\0e\0\0\0l", 16) << (QString(QChar(0xfeff)) + QString::fromLatin1("hel"))
        << true;

    QTest::newRow("utf32-le bom le") << QByteArray("UTF-32LE") << static_cast<int>(QTextCodec::DefaultConversion)
                                     << QByteArray("\xff\xfe\0\0h\0\0\0e\0\0\0l\0\0\0", 16)
                                     << QString::fromLatin1("hel") << true;
    QTest::newRow("utf32-le nobom") << QByteArray("UTF-32LE") << static_cast<int>(QTextCodec::DefaultConversion)
                                    << QByteArray("h\0\0\0e\0\0\0l\0\0\0", 12) << QString::fromLatin1("hel") << true;
    QTest::newRow("utf32-le bom le ignore header")
        << QByteArray("UTF-32LE") << static_cast<int>(QTextCodec::IgnoreHeader)
        << QByteArray("\xff\xfe\0\0h\0\0\0e\0\0\0l\0\0\0", 16) << (QString(QChar(0xfeff)) + QString::fromLatin1("hel"))
        << true;
}
...
void TST_QTextCodec::utfHeaders()
{
    ...
    QLatin1String ignoreReverseTestOn = (QSysInfo::ByteOrder == QSysInfo::BigEndian) ? QLatin1String(" le")
                                                                                     : QLatin1String(" be");
    QString rowName(QTest::currentDataTag());

    if (toUnicode)
    {
        QString result = codec->toUnicode(encoded.constData(), static_cast<int>(encoded.size()), &state);
        ...
        if (!rowName.endsWith("nobom") && !rowName.contains(ignoreReverseTestOn))
        {
            ...
            QByteArray reencoded = codec->fromUnicode(unicode.unicode(), static_cast<int>(unicode.size()), &state2);
            QCOMPARE(reencoded, encoded);
        }
    }
    else
    {
        QByteArray result = codec->fromUnicode(unicode.unicode(), static_cast<int>(unicode.size()), &state);
...
void TST_QTextCodec::codecForHtml_data()
{
    ...
    html = "<!DOCTYPE html><html><head><meta charset=\"utf-8\"><meta http-equiv=\"X-UA-Compatible\" "
           "content=\"IE=9,chrome=1\"><title>Test</title></head>";
    QTest::newRow("UTF-8, no default") << html << noDefault << 106;

    html = "<!DOCTYPE html><html><head><meta charset=\"ISO_8859-1:1987\"><meta http-equiv=\"X-UA-Compatible\" "
           "content=\"IE=9,chrome=1\"><title>Test</title></head>";
    QTest::newRow("latin 1, no default") << html << noDefault << 4;

    html = "<!DOCTYPE html><html><head><meta http-equiv=\"X-UA-Compatible\" content=\"IE=9,chrome=1\"><meta "
           "charset=\"utf-8\"><title>Test</title></head>";
    ...
    html = "<meta http-equiv=\"content-type\" content=\"text/html; charset=utf-8\"><span style=\"color: rgb(0, 0, 0); "
           "font-family: "
           "'Galatia SIL'; font-size: 27px; font-style: normal; font-variant: normal; font-weight: normal; "
           "letter-spacing: normal; "
           "line-height: normal; orphans: auto; text-align: start; text-indent: 0px; text-transform: none; "
           "white-space: normal; widows: "
           "auto; word-spacing: 0px; -webkit-text-size-adjust: auto; -webkit-text-stroke-width: 0px; display: inline "
           "!important; float: "
           "none;\">&#x37b</span>\000";
...
void TST_QTextCodec::codecForUtfText_data()
{
    ...
    QTest::newRow("utf8 bom") << QByteArray("\xef\xbb\xbfhello") << true << 106;
    QTest::newRow("utf8 nobom") << QByteArray("hello") << false << 0;

    QTest::newRow("utf16 bom be") << QByteArray("\xfe\xff\0h\0e\0l", 8) << true << 1013;
    QTest::newRow("utf16 bom le") << QByteArray("\xff\xfeh\0e\0l\0", 8) << true << 1014;
    QTest::newRow("utf16 nobom") << QByteArray("\0h\0e\0l", 6) << false << 0;

    QTest::newRow("utf32 bom be") << QByteArray("\0\0\xfe\xff\0\0\0h\0\0\0e\0\0\0l", 16) << true << 1018;
    QTest::newRow("utf32 bom le") << QByteArray("\xff\xfe\0\0h\0\0\0e\0\0\0l\0\0\0", 16) << true << 1019;
    QTest::newRow("utf32 nobom") << QByteArray("\0\0\0h\0\0\0e\0\0\0l", 12) << false << 0;
}
...
{
    QProcess process;
    process.start(QStringLiteral(ECHO_HELPER_BUILDDIR));
    QString string(QChar(0x410));
    process.write(reinterpret_cast<const char *>(string.utf16()), string.size() * 2);
...
void TST_QTextCodec::moreToFromUnicode_data()
{
    ...
    QTest::newRow("russian")
        << QByteArray("ISO-8859-5")
        << QByteArray("\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0A\x0B\x0C\x0D\x0E\x0F\x10\x11\x12\x13\x14\x15\x16\x17\x18"
                      ...
                      "\x91\x92\x93\x94\x95\x96\x97\x98\x99\x9A\x9B\x9C\x9D\x9E\x9F\xA0\xA1\xA2\xA3\xA4\xA5\xA6\xA7\xA8"
                      "\xA9\xAA\xAB\xAC\xAD\xAE\xAF\xB0\xB1\xB2\xB3\xB4\xB5\xB6\xB7\xB8\xB9\xBA\xBB\xBC\xBD\xBE\xBF\xC0"
                      "\xC1\xC2\xC3\xC4\xC5\xC6\xC7\xC8\xC9\xCA\xCB\xCC\xCD\xCE\xCF\xD0\xD1\xD2\xD3\xD4\xD5\xD6\xD7\xD8"
                      "\xD9\xDA\xDB\xDC\xDD\xDE\xDF\xE0\xE1\xE2\xE3\xE4\xE5\xE6\xE7\xE8\xE9\xEA\xEB\xEC\xED\xEE\xEF\xF0"
                      "\xF1\xF2\xF3\xF4\xF5\xF6\xF7\xF8\xF9\xFA\xFB\xFC\xFD\xFE\xFF\x00");

    QTest::newRow("arabic")
        << QByteArray("ISO-8859-6")
        << QByteArray("\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0A\x0B\x0C\x0D\x0E\x0F\x10\x11\x12\x13\x14\x15\x16\x17\x18"
                      "\x19\x1A\x1B\x1C\x1D\x1E\x1F\x20\x21\x22\x23\x24\x25\x26\x27\x28\x29\x2A\x2B\x2C\x2D\x2E\x2F\x30"
                      "\x31\x32\x33\x34\x35\x36\x37\x38\x39\x3A\x3B\x3C\x3D\x3E\x3F\x40\x41\x42\x43\x44\x45\x46\x47\x48"
                      "\x49\x4A\x4B\x4C\x4D\x4E\x4F\x50\x51\x52\x53\x54\x55\x56\x57\x58\x59\x5A\x5B\x5C\x5D\x5E\x5F\x60"
                      "\x61\x62\x63\x64\x65\x66\x67\x68\x69\x6A\x6B\x6C\x6D\x6E\x6F\x70\x71\x72\x73\x74\x75\x76\x77\x78"
                      "\x79\x7A\x7B\x7C\x7D\x7E\x7F\x80\x81\x82\x83\x84\x85\x86\x87\x88\x89\x8A\x8B\x8C\x8D\x8E\x8F\x90"
                      "\x91\x92\x93\x94\x95\x96\x97\x98\x99\x9A\x9B\x9C\x9D\x9E\x9F\xA0\xA4\xAC\xAD\xBB\xBF\xC1\xC2\xC3"
                      "\xC4\xC5\xC6\xC7\xC8\xC9\xCA\xCB\xCC\xCD\xCE\xCF\xD0\xD1\xD2\xD3\xD4\xD5\xD6\xD7\xD8\xD9\xDA\xE0"
                      "\xE1\xE2\xE3\xE4\xE5\xE6\xE7\xE8\xE9\xEA\xEB\xEC\xED\xEE\xEF\xF0\xF1\xF2");

    QTest::newRow("greek")
        << QByteArray("ISO-8859-7")
        << QByteArray(
               ...
               "\x97\x98\x99\x9A\x9B\x9C\x9D\x9E\x9F\xA0\xA1\xA2\xA3\xA6\xA7\xA8\xA9\xAB\xAC\xAD\xAF\xB0\xB1\xB2\xB3"
               "\xB4\xB5\xB6\xB7\xB8\xB9\xBA\xBB\xBC\xBD\xBE\xBF\xC0\xC1\xC2\xC3\xC4\xC5\xC6\xC7\xC8\xC9\xCA\xCB\xCC"
               "\xCD\xCE\xCF\xD0\xD1\xD3\xD4\xD5\xD6\xD7\xD8\xD9\xDA\xDB\xDC\xDD\xDE\xDF\xE0\xE1\xE2\xE3\xE4\xE5\xE6"
               "\xE7\xE8\xE9\xEA\xEB\xEC\xED\xEE\xEF\xF0\xF1\xF2\xF3\xF4\xF5\xF6\xF7\xF8\xF9\xFA\xFB\xFC\xFD\xFE");

    QTest::newRow("turkish")
        << QByteArray("ISO-8859-9")
        << QByteArray("\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0A\x0B\x0C\x0D\x0E\x0F\x10\x11\x12\x13\x14\x15\x16\x17"
    ...
    for (int i = 1; i <= 0x7f; ++i)
    {
        if (i != '\x1b')
        {
            sms7bit_ba.append(static_cast<char>(i));
        }
    }
    ...
    QTest::newRow("latin3")
        ...
        << QByteArray(
               ...
               "\x65\x66\x67\x68\x69\x6A\x6B\x6C\x6D\x6E\x6F\x70\x71\x72\x73\x74\x75\x76\x77\x78\x79\x7A\x7B\x7C\x7D"
               "\x7E\x7F\x80\x81\x82\x83\x84\x85\x86\x87\x88\x89\x8A\x8B\x8C\x8D\x8E\x8F\x90\x91\x92\x93\x94\x95\x96"
               "\x97\x98\x99\x9A\x9B\x9C\x9D\x9E\x9F\xA0\xA1\xA2\xA3\xA4\xA6\xA7\xA8\xA9\xAA\xAB\xAC\xAD\xAF\xB0\xB1"
               "\xB2\xB3\xB4\xB5\xB6\xB7\xB8\xB9\xBA\xBB\xBC\xBD\xBF\xC0\xC1\xC2\xC4\xC5\xC6\xC7\xC8\xC9\xCA\xCB\xCC"
               "\xCD\xCE\xCF\xD1\xD2\xD3\xD4\xD5\xD6\xD7\xD8\xD9\xDA\xDB\xDC\xDD\xDE\xDF\xE0\xE1\xE2\xE4\xE5\xE6\xE7"
               "\xE8\xE9\xEA\xEB\xEC\xED\xEE\xEF\xF1\xF2\xF3\xF4\xF5\xF6\xF7\xF8\xF9\xFA\xFB\xFC\xFD\xFE\xFF");

    QTest::newRow("latin4")
        << QByteArray("ISO-8859-4")
        << QByteArray("\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0A\x0B\x0C\x0D\x0E\x0F\x10\x11\x12\x13\x14\x15\x16\x17\x18"
                      "\x19\x1A\x1B\x1C\x1D\x1E\x1F\x20\x21\x22\x23\x24\x25\x26\x27\x28\x29\x2A\x2B\x2C\x2D\x2E\x2F\x30"
                      "\x31\x32\x33\x34\x35\x36\x37\x38\x39\x3A\x3B\x3C\x3D\x3E\x3F\x40\x41\x42\x43\x44\x45\x46\x47\x48"
                      "\x49\x4A\x4B\x4C\x4D\x4E\x4F\x50\x51\x52\x53\x54\x55\x56\x57\x58\x59\x5A\x5B\x5C\x5D\x5E\x5F\x60"
    ...
    QTest::newRow("cp850")
        ...
        << QByteArray(
               ...
               "\x39\x3a\x3b\x3c\x3d\x3e\x3f\x40\x41\x42\x43\x44\x45\x46\x47\x48\x49\x4a\x4b\x4c\x4d\x4e\x4f\x50\x51"
               "\x52\x53\x54\x55\x56\x57\x58\x59\x5a\x5b\x5c\x5d\x5e\x5f\x60\x61\x62\x63\x64\x65\x66\x67\x68\x69\x6a"
x01\x02\x03\x04\x05\x06\x07\x08\x09\x0A\x0B\x0C\x0D\x0E\x0F\x10\x11\x12\x13\x14\x15\x16\x17\x18"
                      ...
                      "\x49\x4A\x4B\x4C\x4D\x4E\x4F\x50\x51\x52\x53\x54\x55\x56\x57\x58\x59\x5A\x5B\x5C\x5D\x5E\x5F\x60"
                      "\x61\x62\x63\x64\x65\x66\x67\x68\x69\x6A\x6B\x6C\x6D\x6E\x6F\x70\x71\x72\x73\x74\x75\x76\x77\x78"
                      "\x79\x7A\x7B\x7C\x7D\x7E\x7F\x80\x82\x83\x84\x85\x86\x87\x89\x8B\x91\x92\x93\x94\x95\x96\x97\x99"
                      "\x9B\xA0\xA1\xA2\xA3\xA4\xA5\xA6\xA7\xA8\xA9\xAB\xAC\xAD\xAE\xAF\xB0\xB1\xB2\xB3\xB4\xB5\xB6\xB7"
                      "\xB8\xB9\xBA\xBB\xBC\xBD\xBE\xBF\xC0\xC1\xC2\xC3\xC4\xC5\xC6\xC7\xC8\xC9\xCA\xCB\xCC\xCD\xCE\xCF"
                      "\xD0\xD1\xD3\xD4\xD5\xD6\xD7\xD8\xD9\xDA\xDB\xDC\xDD\xDE\xDF\xE0\xE1\xE2\xE3\xE4\xE5\xE6\xE7\xE8"
                      "\xE9\xEA\xEB\xEC\xED\xEE\xEF\xF0\xF1\xF2\xF3\xF4\xF5\xF6\xF7\xF8\xF9\xFA\xFB\xFC\xFD\xFE");
    ...
    QTest::newRow("cp1254")
        ...
        << QByteArray(
               ...
               "\x4C\x4D\x4E\x4F\x50\x51\x52\x53\x54\x55\x56\x57\x58\x59\x5A\x5B\x5C\x5D\x5E\x5F\x60\x61\x62\x63\x64"
               "\x65\x66\x67\x68\x69\x6A\x6B\x6C\x6D\x6E\x6F\x70\x71\x72\x73\x74\x75\x76\x77\x78\x79\x7A\x7B\x7C\x7D"
               "\x7E\x7F\x80\x82\x83\x84\x85\x86\x87\x88\x89\x8A\x8B\x8C\x91\x92\x93\x94\x95\x96\x97\x98\x99\x9A\x9B"
               "\x9C\x9F\xA0\xA1\xA2\xA3\xA4\xA5\xA6\xA7\xA8\xA9\xAA\xAB\xAC\xAD\xAE\xAF\xB0\xB1\xB2\xB3\xB4\xB5\xB6"
               "\xB7\xB8\xB9\xBA\xBB\xBC\xBD\xBE\xBF\xC0\xC1\xC2\xC3\xC4\xC5\xC6\xC7\xC8\xC9\xCA\xCB\xCC\xCD\xCE\xCF"
               "\xD0\xD1\xD2\xD3\xD4\xD5\xD6\xD7\xD8\xD9\xDA\xDB\xDC\xDD\xDE\xDF\xE0\xE1\xE2\xE3\xE4\xE5\xE6\xE7\xE8"
    ...
    QTest::newRow("cp1255")
        ...
        << QByteArray(
               ...
               "\x4C\x4D\x4E\x4F\x50\x51\x52\x53\x54\x55\x56\x57\x58\x59\x5A\x5B\x5C\x5D\x5E\x5F\x60\x61\x62\x63\x64"
               "\x65\x66\x67\x68\x69\x6A\x6B\x6C\x6D\x6E\x6F\x70\x71\x72\x73\x74\x75\x76\x77\x78\x79\x7A\x7B\x7C\x7D"
               "\x7E\x7F\x80\x82\x83\x84\x85\x86\x87\x88\x89,"
               "x8B\x91\x92\x93\x94\x95\x96\x97\x98\x99\x9B\xA0\xA1\xA2\xA3\xA4\xA5\xA6\xA7\xA8\xA9\xAA\xAB\xAC\xAD\xAE"
               "\xD2\xD3\xD4\xD5\xD6\xD7\xD8\xD9\xDA\xDB\xDC\xDD\xDE\xDF\xE0\xE1\xE2\xE3\xE4\xE5\xE6\xE7\xE8\xE9\xEA"
    ...
    for (int x = 0x20; x <= 0xff; ++x)
    {
        if (x != 0x9A && x != 0xbf)
        {
            koi8_r_ba.append(static_cast<char>(x));
        }
    }
    ...
    for (int x = 0x20; x <= 0xff; ++x)
    {
        koi8_u_ba.append(static_cast<char>(x));
    }
    ...
    for (unsigned char u = 0xa1; u <= 0xf9; u++)
    {
        ...
        {
            big5_ba.append(static_cast<char>(u));
            big5_ba.append(static_cast<char>(v));
        }
        unsigned char v_up;
        switch (u)
        {
    ...
    for (unsigned char u = 0xa1; u <= 0xf7; u++)
    {
        ...
        {
            gb2312_ba.append(static_cast<char>(u));
            gb2312_ba.append(static_cast<char>(v));
        }
    }
...
#else
    QSKIP("Support for big codecs disabled");
#endif
}
...
struct UserCodec : public QTextCodec
{
    ...
    QByteArray name() const override { return "UserCodec"; }
    QList<QByteArray> aliases() const override { return QList<QByteArray>() << "usercodec" << "user-codec"; }
    int mibEnum() const override { return 5000; }

    virtual QString convertToUnicode(const char *, int, ConverterState *) const override { return QString(); }
    virtual QByteArray convertFromUnicode(const QChar *, int, ConverterState *) const override { return QByteArray(); }
};
...
void TST_QTextCodec::retainNullness()
{
    ...
    const QByteArray ba = length >= 0 ? QByteArray(input, length) : QByteArray(input);
    const QString unicode = length >= 0 ? codec->toUnicode(input, length) : codec->toUnicode(input);
...
void TST_QTextCodec::canEncode_data()
{
    ...
    QTest::newRow("English ISO-8859-1") << "ISO-8859-1" << "Hello World" << QByteArray("Hello World") << true;
#if defined(WITH_ICU_CODECS) || (defined(WITH_BIG_CODECS) && !defined(Q_OS_INTEGRITY))
    QTest::newRow("English big5") << "Big5" << "Hello World" << QByteArray("Hello World") << true;
#endif

    QTest::newRow("Greek win1252") << "Windows-1252"
                                   << QString("\u03c0\u03bf\u03bb\u03cd\u03c4\u03c1\u03bf\u03c0\u03bf\u03bd")
                                   << QByteArray("??????????") << false;
    QTest::newRow("Greek win1253") << "Windows-1253"
                                   << QString("\u03c0\u03bf\u03bb\u03cd\u03c4\u03c1\u03bf\u03c0\u03bf\u03bd")
                                   << QByteArray("\xF0\xEF\xEB\xFD\xF4\xF1\xEF\xF0\xEF\xED") << true;

    QTest::newRow("Russian win1252") << "Windows-1252"
                                     << QString("\u041f\u0440\u0438\u0432\u0435\u0442 \u043c\u0438\u0440")
                                     << QByteArray("?????? ???") << false;
    QTest::newRow("Russian win1251") << "Windows-1251"
                                     << QString("\u041f\u0440\u0438\u0432\u0435\u0442 \u043c\u0438\u0440")
                                     << QByteArray("\xCF\xF0\xE8\xE2\xE5\xF2 \xEC\xE8\xF0") << true;

    QTest::newRow("English from ucs4") << "ISO-8859-1" << QString("\u0048\u0065\u006c\u006c\u006f\u0021")
    ...
    QTest::newRow("With special chars") << "ISO-8859-1" << QString("\u0001\u0002\u0003\u0008\u0009\u000a\u000b\u000d")
