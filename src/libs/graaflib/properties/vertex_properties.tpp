...
std::size_t vertex_degree(const graaf::graph<V, E, T> &graph, vertex_id_t vertex_id)
{
...
template<typename V, typename E, graph_type T>
std::size_t vertex_outdegree(const graaf::graph<V, E, T> &graph, vertex_id_t vertex_id)
{
    return (graph.get_neighbors(vertex_id)).size();
}

template<typename V, typename E, graph_type T>
std::size_t vertex_indegree(const graaf::graph<V, E, T> &graph, vertex_id_t vertex_id)
{
    ...
    if constexpr (T == graph_type::DIRECTED)
    {
        return static_cast<std::size_t>(
            std::ranges::count_if(graph.get_vertices(),
