#pragma once
#include "../graph_traversal/depth_first_search.h"
#include "../utils.h"
#include "kosaraju.h"
...
template<typename V, typename E>
void make_strongly_connected_component_from_vertex(const vertex_id_t vertex,
                                                   const directed_graph<V, E> &transposed_graph,
                                                   ...
                                                   std::unordered_set<vertex_id_t> &seen_vertices)
{
    if (!seen_vertices.contains(vertex))
    {
        ...
        {
            make_strongly_connected_component_from_vertex(neighbour, transposed_graph, scc, seen_vertices);
        }
    }
}
...
sccs_t kosarajus_strongly_connected_components(const directed_graph<V, E> &graph)
{
    ...
    while (!stack.empty())
    {
        ...
        {
            scc.clear();
            make_strongly_connected_component_from_vertex(stack.top(), transposed_graph, scc, seen_vertices);
            sccs.push_back(scc);
        }
