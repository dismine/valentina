#pragma once
#include "../cycle_detection/dfs_cycle_detection.h"
...
void do_dfs_topological_sort(const graph<V, E, graph_type::DIRECTED> &graph,
                             ...
                             std::vector<vertex_id_t> &sorted_vertices)
{
    ...
    for (const auto &next_vertex : graph.get_neighbors(start_vertex))
    {
        ...
        {
            do_dfs_topological_sort(graph, next_vertex, processed_vertices, sorted_vertices);
        }
    }
...
template<typename V, typename E>
std::optional<std::vector<vertex_id_t>> dfs_topological_sort(const graph<V, E, graph_type::DIRECTED> &graph)
{
