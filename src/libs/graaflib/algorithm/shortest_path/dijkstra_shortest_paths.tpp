...
[[nodiscard]] std::unordered_map<vertex_id_t, graph_path<WEIGHT_T>> dijkstra_shortest_paths(const graph<V, E, T> &graph,
                                                                                            vertex_id_t source_vertex)
{
    ...
    while (!to_explore.empty())
    {
        ...
        if (shortest_paths.contains(current.id) && current.dist_from_start > shortest_paths[current.id].total_weight)
        {
            continue;
        }
        ...
        for (const auto neighbor : graph.get_neighbors(current.id))
        {
            ...
            if (edge_weight < 0)
            {
                ...
                error_msg << "Negative edge weight [" << edge_weight << "] between vertices [" << current.id << "] -> ["
                          << neighbor << "].";
            ...
            if (!shortest_paths.contains(neighbor) || distance < shortest_paths[neighbor].total_weight)
            {
