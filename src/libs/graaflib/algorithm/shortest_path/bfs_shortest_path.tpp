#pragma once
#include "../graph_traversal/breadth_first_search.h"
...
std::optional<graph_path<WEIGHT_T>> bfs_shortest_path(const graph<V, E, T> &graph,
    ...
    const auto callback{[&vertex_info](const edge_id_t &edge)
                        {
                            ...
                            {
                                vertex_info[target] = {target, vertex_info[source].dist_from_start + 1, source};
                            }
                        }};

    // We keep searching until we have reached the target vertex
    const auto search_termination_strategy{[end_vertex](const vertex_id_t vertex_id)
