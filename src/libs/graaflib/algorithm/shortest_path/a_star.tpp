...
requires std::is_invocable_r_v<WEIGHT_T, HEURISTIC_T &, vertex_id_t> std::optional<graph_path<WEIGHT_T>> a_star_search(
    const graph<V, E, T> &graph, vertex_id_t start_vertex, vertex_id_t target_vertex, const HEURISTIC_T &heuristic)
{
    ...
    using a_star_queue_t = std::priority_queue<weighted_path_item, std::vector<weighted_path_item>, std::greater<>>;
    ...
    vertex_info[start_vertex] = {start_vertex,
    ...
    while (!open_set.empty())
    {
        ...
        for (const auto &neighbor : graph.get_neighbors(current.id))
        {
            ...
            if (edge_weight < 0)
            {
                throw std::invalid_argument{"Negative edge weight [" + std::to_string(edge_weight)
