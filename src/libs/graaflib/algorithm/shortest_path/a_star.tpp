...
template <typename V,
          ...
          typename WEIGHT_T
          GRAAF_ENABLE_IF(std::is_invocable_r_v<WEIGHT_T, HEURISTIC_T&, vertex_id_t>)
>
GRAAF_REQUIRES(std::is_invocable_r_v<WEIGHT_T, HEURISTIC_T&, vertex_id_t>)
// clang-format on
                                                  ...
                                                  const HEURISTIC_T &heuristic)
{
    ...
    while (!open_set.empty())
    {
        ...
        for (const auto &neighbor : graph.get_neighbors(current.id))
        {
            ...
            if (edge_weight < 0)
            {
                throw std::invalid_argument{fmt::format("Negative edge weight [{}] between vertices [{}] -> [{}].",
                                                        edge_weight,
                                                        current.id,
                                                        neighbor)};
            }
