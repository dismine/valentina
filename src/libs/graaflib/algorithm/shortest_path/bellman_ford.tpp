...
std::unordered_map<vertex_id_t, graph_path<WEIGHT_T>> bellman_ford_shortest_paths(const graph<V, E, T> &graph,
                                                                                  vertex_id_t start_vertex)
{
    std::unordered_map<vertex_id_t, graph_path<WEIGHT_T>> shortest_paths;

    const auto found_shorter_path{[&shortest_paths](edge_id_t edge_id, const E &edge)
                                  {
                                      const auto weight = get_weight(edge);
                                      const auto [u, v]{edge_id};
                                      return shortest_paths[u].total_weight != std::numeric_limits<WEIGHT_T>::max()
