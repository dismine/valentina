...
struct prim_candidate_edge
{
    ...
    [[nodiscard]] bool operator>(const prim_candidate_edge<WEIGHT_T> &other) const { return weight > other.weight; }
};

}; // namespace detail

template<typename V, typename E, typename WEIGHT_T>
std::optional<std::vector<edge_id_t>> prim_minimum_spanning_tree(const graph<V, E, graph_type::UNDIRECTED> &graph,
                                                                 vertex_id_t start_vertex)
{
    ...
    std::priority_queue<candidate_edge, std::vector<candidate_edge>, std::greater<>> to_explore{};

    const auto push_edges_from{
        [&graph, &in_mst, &to_explore](vertex_id_t from)
        {
            for (const auto neighbor : graph.get_neighbors(from))
            {
                if (!in_mst.contains(neighbor))
                {
                    to_explore.push(candidate_edge{from, neighbor, get_weight(graph.get_edge(from, neighbor))});
                }
            }
        }};
