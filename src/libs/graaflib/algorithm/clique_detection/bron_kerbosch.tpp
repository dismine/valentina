...
vertex_set_t do_get_intersection(const vertex_set_t &lhs, const vertex_set_t &rhs)
{
                                     ...
                                     const graph<V, E, graph_type::UNDIRECTED> &graph)
{
    ...
    const auto pivot_vertex = *std::ranges::max_element(union_set,
                                                        {},
                                                        [&graph](vertex_id_t id)
                                                        { return graph.get_neighbors(id).size(); });

    // vertices_to_process = candidate_vertices \ N(pivot_vertex)
    auto vertices_to_process = set_difference(vertices, graph.get_neighbors(pivot_vertex));
    ...
    for (const auto &vertex : vertices_to_process)
    {
        ...
        exclude_intersection = do_get_intersection(excluded_vertices, vertex_neighbors);

        clique.push_back(vertex);
        do_bron_kerbosch_maximal_clique(clique, vertices_intersection, exclude_intersection, maximal_cliques, graph);
        clique.pop_back();
...
std::vector<std::vector<vertex_id_t>> bron_kerbosch(const graph<V, E, graph_type::UNDIRECTED> &graph)
{
    ...
    detail::do_bron_kerbosch_maximal_clique(clique, vertices, excluded, maximal_cliques, graph);
