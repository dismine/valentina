...
template<typename V,
         ...
         GRAAF_ENABLE_IF(std::is_invocable_r_v<std::string, const VERTEX_WRITER_T &, vertex_id_t, const V &> &&
                         std::is_invocable_r_v<std::string, const EDGE_WRITER_T &, const graaf::edge_id_t &, const typename graph<V, E, T>::edge_t &>)
>
GRAAF_REQUIRES(std::is_invocable_r_v<std::string, const VERTEX_WRITER_T &, vertex_id_t, const V &> &&
               std::is_invocable_r_v<std::string, const EDGE_WRITER_T &, const graaf::edge_id_t &, const typename graph<V, E, T>::edge_t &>)
// clang-format on
            ...
            const EDGE_WRITER_T &edge_writer)
{
    ...
    const auto append_line{[&dot_file](const std::string &line) { dot_file << line << std::endl; }};
    ...
    {
        append_line("\t" + std::to_string(vertex_id) + " [" + vertex_writer(vertex_id, vertex) + "];");
    }
    ...
    for (const auto &[edge_id, edge] : graph.get_edges())
    {
        ...
        append_line("\t" + std::to_string(source_id) + " " + edge_specifier + " " + std::to_string(target_id) + " ["
                    + edge_writer(edge_id, edge) + "];");
    }
