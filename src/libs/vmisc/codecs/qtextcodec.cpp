...
#if defined(WITH_BIG_CODECS) && defined(WITH_TEXTCODEC)
...
#endif // WITH_BIG_CODECS

#include <clocale>

#endif // WITH_ICU_CODECS
...
{
public:
    explicit TextCodecsMutexLocker(QRecursiveMutex *mutex = textCodecsMutex())
      : m_mutex(mutex)
    {
        if (m_mutex)
        {
    ...
    QRecursiveMutex *mutex() const { return m_mutex; }
...
static QTextCodec *checkForCodec(const QByteArray &name) {
    ...
    if (!c) {
        const vsizetype index = name.indexOf('@');
        if (index != -1) {
...
// Backport QCoreApplicationPrivate::initLocale
static void initLocale()
{
    ...
    if (!Q_LIKELY(qstricmp(oldEncoding.data(), "UTF-8") == 0 || qstricmp(oldEncoding.data(), "utf8") == 0))
    {
        ...
        warnOnOverride = qstrcmp(setlocale(LC_CTYPE, nullptr), "C") != 0 || getenv("LC_ALL") || getenv("LC_CTYPE")
                         || getenv("LANG");
...
static QTextCodec *setupLocaleMapper()
{
    ...
    if (!locale) {
        ...
        // 1. CODESET from ctype if it contains a .CODESET part (e.g. en_US.ISO8859-15)
        vsizetype indexOfDot = ctype.indexOf('.');
        if (indexOfDot != -1)
...
#endif // WITH_BASIC_CODECS
    for (int i = 0; i < QSimpleTextCodec::numSimpleCodecs; ++i)
        (void)new QSimpleTextCodec(i);

#if defined(WITH_BIG_CODECS) && !defined(Q_OS_INTEGRITY)
...
#endif // WITH_BIG_CODECS && !Q_OS_INTEGRITY
#if defined(WITH_ICONV_CODECS)
    (void) new QIconvCodec;
#endif
...
QByteArray QTextCodec::fromUnicode(const QString& str) const
{
    ...
    return convertFromUnicode(str.constData(), static_cast<int>(str.size()), &state);
}
...
{
    ConverterState state = DefaultConversion | Flag::Stateless;
    return convertFromUnicode(str.data(), static_cast<int>(str.size()), &state);
}
...
{
    ConverterState state = DefaultConversion | Flag::Stateless;
    return convertToUnicode(a.constData(), static_cast<int>(a.size()), &state);
}
...
bool QTextCodec::canEncode(const QString& s) const
{
    ConverterState state;
    state.flags = ConvertInvalidToNull;
    convertFromUnicode(s.constData(), static_cast<int>(s.size()), &state);
    return (state.invalidChars == 0);
}
...
bool QTextCodec::canEncode(QStringView s) const
{
    ConverterState state;
    state.flags = ConvertInvalidToNull;
    convertFromUnicode(s.data(), static_cast<int>(s.size()), &state);
...
{
    return c->fromUnicode(str.constData(), static_cast<int>(str.size()), &state);
}
...
{
    return c->fromUnicode(str.data(), static_cast<int>(str.size()), &state);
}
...
void QTextDecoder::toUnicode(QString *target, const char *chars, int len)
{
    ...
    case 4: // latin1
        target->resize(len);
        qt_from_latin1(reinterpret_cast<char16_t *>(target->data()), chars, static_cast<size_t>(len));
        break;
    default:
...
{
    return c->toUnicode(ba.constData(), static_cast<int>(ba.size()), &state);
}
