...
static const struct {
    ...
    { "ISO-8859-6", { "ECMA-114", "ASMO-708", "arabic", "iso-ir-127", "csISOLatinArabic", nullptr }, 9,
      { 0x0080, 0x0081, 0x0082, 0x0083, 0x0084, 0x0085, 0x0086, 0x0087,
        0x0088, 0x0089, 0x008A, 0x008B, 0x008C, 0x008D, 0x008E, 0x008F,
        0x0090, 0x0091, 0x0092, 0x0093, 0x0094, 0x0095, 0x0096, 0x0097,
        0x0098, 0x0099, 0x009A, 0x009B, 0x009C, 0x009D, 0x009E, 0x009F,
    ...
    { "ISO-8859-8", { "iso-ir-138", "hebrew", "csISOLatinHebrew", nullptr }, 11,
      { 0x0080, 0x0081, 0x0082, 0x0083, 0x0084, 0x0085, 0x0086, 0x0087,
        0x0088, 0x0089, 0x008A, 0x008B, 0x008C, 0x008D, 0x008E, 0x008F,
        0x0090, 0x0091, 0x0092, 0x0093, 0x0094, 0x0095, 0x0096, 0x0097,
        0x0098, 0x0099, 0x009A, 0x009B, 0x009C, 0x009D, 0x009E, 0x009F,
...
static QByteArray *buildReverseMap(int forwardIndex)
{
    ...
    for(i=128; i<256; i++) {
        ...
        if (u < m)
            (*map)[u] = static_cast<char>(static_cast<unsigned char>(i));
    }
    return map;
}

QString QSimpleTextCodec::convertToUnicode(const char *chars, int len, ConverterState *state) const
{
    Q_UNUSED(state)
    const auto *c = reinterpret_cast<const unsigned char *>(chars);
    ...
    for (int i = 0; i < len; i++) {
        if (c[i] > 127)
            ...
            uc[i] = QLatin1Char(static_cast<char>(c[i]));
    }
    return r;
}

QByteArray QSimpleTextCodec::convertFromUnicode(const QChar *in, int len, ConverterState *state) const
{
    ...
    int u;
    const QChar* ucp = in;
    auto *rp = reinterpret_cast<unsigned char *>(r.data());
    const auto *rmp = reinterpret_cast<const unsigned char *>(rmap->constData());
    auto rmsize = static_cast<int>(rmap->size());
    while(i--)
    {
        u = ucp->unicode();
        if (u < 128) {
