...
QByteArray QIsciiCodec::convertFromUnicode(const QChar *uc, int len, ConverterState *state) const
{
        ...
        if (pos > 0 && pos < 0x80) {
            ...
            if (iscii > 0x80) {
                *ch++ = iscii;
                ...
                Q_ASSERT(static_cast<size_t>(2 * iscii) < (sizeof(uni_to_iscii_pairs) / sizeof(uni_to_iscii_pairs[0])));
                const uchar *pair = uni_to_iscii_pairs + 2*iscii;
                *ch++ = *pair++;
                *ch++ = *pair++;
            } else {
                *ch++ = static_cast<uchar>(replacement);
                ++invalid;
            }
        } else {
    ...
    result.truncate(ch - reinterpret_cast<uchar *>(result.data()));
...
QString QIsciiCodec::convertToUnicode(const char* chars, int len, ConverterState *state) const
{
    ...
    for (int i = 0; i < len; ++i) {
        char16_t ch = static_cast<uchar>(chars[i]);
        if (ch < 0xa0)
            *uc++ = ch;
        else {
        ...
        halant = (static_cast<uchar>(chars[i]) == 0xe8);
    }
