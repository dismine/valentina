...
#if (defined(__SSE2__) && defined(QT_COMPILER_SUPPORTS_SSE2)) || (defined(__ARM_NEON__) && defined(Q_PROCESSOR_ARM_64))
...
static inline bool simdEncodeAscii(uchar *&dst, const ushort *&nextAscii, const ushort *&src, const ushort *end)
{
    ...
    for (; end - src >= 16; src += 16, dst += 16)
    {
        ...
        __m128i data2 = _mm_loadu_si128(1 + reinterpret_cast<const __m128i *>(src));
#endif
        ...
        auto n = static_cast<ushort>(~_mm_movemask_epi8(nonAscii));
        if (n)
        {
            ...
            n = static_cast<ushort>(qCountTrailingZeroBits(n));
            dst += n;
            src += n;
            return false;
        }
    ...
    if (end - src >= 8)
    {
        ...
        auto n = static_cast<uchar>(~_mm_movemask_epi8(nonAscii));
        if (n)
        {
            nextAscii = src + qBitScanReverse(n) + 1;
            n = static_cast<uchar>(qCountTrailingZeroBits(n));
            dst += n;
            src += n;
            return false;
        }
...
static inline bool simdDecodeAscii(ushort *&dst, const uchar *&nextAscii, const uchar *&src, const uchar *end)
{
    ...
    for (; end - src >= 16; src += 16, dst += 16)
    {
        ...
        uint n = static_cast<uint>(_mm_movemask_epi8(data));
        if (!n)
        {
            // unpack
            _mm_storeu_si128(reinterpret_cast<__m128i *>(dst), _mm_unpacklo_epi8(data, _mm_setzero_si128()));
            _mm_storeu_si128(1 + reinterpret_cast<__m128i *>(dst), _mm_unpackhi_epi8(data, _mm_setzero_si128()));
            continue;
        }
#endif
...
static inline const uchar *simdFindNonAscii(const uchar *src, const uchar *end, const uchar *&nextAscii)
{
    ...
    for (; end - src >= 4; src += 4)
    {
        auto data = qFromUnaligned<quint32>(src);
        data &= 0x80808080U;
        if (!data)
            continue;
...
QByteArray QUtf8::convertFromUnicode(const QChar *uc, int len)
{
    ...
    const auto *src = reinterpret_cast<const ushort *>(uc);
...
QByteArray QUtf8::convertFromUnicode(const QChar *uc, int len, QTextCodec::ConverterState *state)
{
    ...
    if (state)
    {
        ...
        if (state->remainingChars)
            surrogate_high = static_cast<int>(state->state_data[0]);
    }
    ...
    while (src != end)
    {
        ...
        if (surrogate_high != -1)
        {
            uc = static_cast<ushort>(surrogate_high);
            surrogate_high = -1;
    ...
    rstr.resize(cursor - reinterpret_cast<const uchar *>(rstr.constData()));
    if (state)
    {
        ...
        if (surrogate_high >= 0)
        {
            ...
            state->state_data[0] = static_cast<uint>(surrogate_high);
        }
    }
    return rstr;
}
...
QChar *QUtf8::convertToUnicode(QChar *buffer, const char *chars, int len) noexcept
{
    ...
    const auto *src = reinterpret_cast<const uchar *>(chars);
    ...
    if (!simdDecodeAscii(dst, nextAscii, src, end))
    {
        ...
        if (Q_UNLIKELY(src == reinterpret_cast<const uchar *>(chars)) && end - src >= 3
...
QString QUtf8::convertToUnicode(const char *chars, int len, QTextCodec::ConverterState *state)
{
    ...
    if (state->remainingChars)
    {
        ...
        int remainingCharsCount = static_cast<int>(state->remainingChars);
        int newCharsToCopy = static_cast<int>(
            qMin<int>(static_cast<int>(sizeof(remainingCharsData)) - remainingCharsCount, end - src));

        memset(remainingCharsData, 0, sizeof(remainingCharsData));
        memcpy(remainingCharsData, &state->state_data[0], static_cast<size_t>(remainingCharsCount));
        memcpy(remainingCharsData + remainingCharsCount, src, static_cast<size_t>(newCharsToCopy));

        const uchar *begin = &remainingCharsData[1];
        res = QUtf8Functions::fromUtf8<QUtf8BaseTraits>(remainingCharsData[0],
                                                        dst,
                                                        begin,
                                                        static_cast<const uchar *>(remainingCharsData)
                                                            + remainingCharsCount + newCharsToCopy);
        ...
        else if (res == QUtf8BaseTraits::EndOfString)
        {
            ...
            memcpy(&state->state_data[0], remainingCharsData, static_cast<size_t>(state->remainingChars));
            return QString();
        }
    ...
    if (res == QUtf8BaseTraits::EndOfString)
    {
        ...
        else
        {
            ...
            memcpy(&state->state_data[0], src, static_cast<size_t>(end - src));
        }
    }
    else
    {
    ...
    result.truncate(dst - reinterpret_cast<const ushort *>(result.unicode()));
    return result;
}
...
{
    uint uc1, uc2;
    const auto *src1 = reinterpret_cast<const uchar *>(utf8);
    const auto *end1 = src1 + u8len;
...
int QUtf8::compareUtf8(const char *utf8, qsizetype u8len, QLatin1String s)
{
    ...
    const auto *src1 = reinterpret_cast<const uchar *>(utf8);
    const auto *end1 = src1 + u8len;
    const auto *src2 = reinterpret_cast<const uchar *>(s.latin1());
    const auto *end2 = src2 + s.size();
...
QString QUtf16::convertToUnicode(const char *chars, int len, QTextCodec::ConverterState *state, DataEndianness e)
{
    ...
    if (state)
    {
        ...
        if (endian == DetectEndianness)
            endian = static_cast<DataEndianness>(state->state_data[Endian]);
        if (state->remainingChars)
        {
            half = true;
            buf = static_cast<uchar>(state->state_data[Data]);
        }
    }
    ...
    auto *qch = reinterpret_cast<QChar *>(result.data());
    while (len--)
    {
        if (half)
        {
            ...
            {
                ch.setRow(static_cast<uchar>(*chars++));
                ch.setCell(buf);
            }
            else
            {
                ch.setRow(buf);
                ch.setCell(static_cast<uchar>(*chars++));
            }
            if (!headerdone)
            {
                headerdone = true;
                if (endian == DetectEndianness)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            endian = LittleEndianness;
                            ch = QChar::fromUcs2(
                                static_cast<char16_t>((ch.unicode() >> 8) | ((ch.unicode() & 0xff) << 8)));
                        }
                        *qch++ = ch;
                    }
                }
...
QByteArray QUtf32::convertFromUnicode(const QChar *uc, int len, QTextCodec::ConverterState *state, DataEndianness e)
{
    ...
    if (!state || !(state->flags & QTextCodec::IgnoreHeader))
    {
        if (endian == BigEndianness)
        {
            data[0] = 0;
            data[1] = 0;
            data[2] = static_cast<char>(0xfe);
            data[3] = static_cast<char>(0xff);
        }
        else
        {
            data[0] = static_cast<char>(0xff);
            data[1] = static_cast<char>(0xfe);
            data[2] = 0;
            data[3] = 0;
        }
        data += 4;
...
QString QUtf32::convertToUnicode(const char *chars, int len, QTextCodec::ConverterState *state, DataEndianness e)
{
    ...
    if (state)
    {
        ...
        num = static_cast<int>(state->remainingChars);
    ...
    while (chars < end)
    {
        tuple[num++] = static_cast<uchar>(*chars++);
        if (num == 4)
        {
            if (!headerdone)
            {
                ...
                else if (((endian == BigEndianness) ? qFromBigEndian<quint32>(tuple) : qFromLittleEndian<quint32>(tuple))
                         == QChar::ByteOrderMark)
                {
                    num = 0;
                    continue;
...
{
    const int arraySize = static_cast<int>(ba.size());
    ...
    if (arraySize > 3)
    {
        ...
        if (uc == qToBigEndian(bom) || uc == qToLittleEndian(bom))
            return QUtf32::convertToUnicode(ba.constData(), static_cast<int>(ba.length()), nullptr); // utf-32
    }
    ...
    if (arraySize > 1)
    {
        ...
        if (uc == qToBigEndian(ushort(bom)) || qToLittleEndian(ushort(bom)))
            return QUtf16::convertToUnicode(ba.constData(), static_cast<int>(ba.length()), nullptr); // utf-16
    }
    return QUtf8::convertToUnicode(ba.constData(), static_cast<int>(ba.length()));
}
