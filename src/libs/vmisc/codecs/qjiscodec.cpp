...
QByteArray QJisCodec::convertFromUnicode(const QChar *uc, int len, ConverterState *cs) const
{
    ...
    for (int i = 0; i < len; i++) {
        ...
        if (ch.row() == 0x00 && ch.cell() < 0x80) {
            ...
            // Invalid
            state = UnknownState;
            j = static_cast<uint>(replacement);
            ++invalid;
        }
        if (state != prev) {
        ...
        if (j < 0x0100) {
            result += static_cast<char>(j & 0xff);
        } else {
            result += static_cast<char>((j >> 8) & 0xff);
            result += static_cast<char>(j & 0xff);
        }
    }
    if (prev != Ascii) {
...
QString QJisCodec::convertToUnicode(const char* chars, int len, ConverterState *cs) const
{
    ...
    if (cs) {
        ...
        state = static_cast<Iso2022State>((cs->state_data[1] >> 0) & 0xff);
        prev = static_cast<Iso2022State>((cs->state_data[1] >> 8) & 0xff);
        esc = cs->state_data[2];
    }
    int invalid = 0;
    ...
    if (cs) {
        ...
        cs->invalidChars += invalid;
        cs->state_data[0] = static_cast<uint>((buf[0] << 24) + (buf[1] << 16) + (buf[2] << 8) + buf[3]);
        cs->state_data[1] = static_cast<uint>((prev << 8) + state);
        cs->state_data[2] = esc;
    }
