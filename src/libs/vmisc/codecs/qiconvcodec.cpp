...
void QIconvCodec::init() const
{
    ...
    Q_ASSERT_X(utf16Codec != nullptr, "QIconvCodec::convertToUnicode", "internal error, UTF-16 codec not found");
    if (!utf16Codec) {
...
void QIconvCodec::IconvState::saveChars(const char *c, int count)
{
    if (count > bufferLen) {
        if (buffer != array)
            delete[] buffer;
        buffer = new char[static_cast<unsigned long>(bufferLen = count)];
    }

    memcpy(buffer, c, static_cast<size_t>(count));
}
...
QString QIconvCodec::convertToUnicode(const char* chars, int len, ConverterState *convState) const
{
    ...
    if (convState) {
        ...
        if (convState->d[0]) {
            // restore state
            remainingCount = static_cast<int>(convState->remainingChars);
    ...
    if (remainingCount) {
        ...
        inBytesLeft += static_cast<unsigned long>(remainingCount);
        in.resize(static_cast<vsizetype>(inBytesLeft));
        inBytes = in.data();

        memcpy(in.data(), remainingBuffer, static_cast<size_t>(remainingCount));
        memcpy(in.data() + remainingCount, chars, static_cast<size_t>(len));
    ...
    do {
        ...
        {
            if (errno == E2BIG) {
                int offset = static_cast<int>(static_cast<unsigned long long>(ba.size()) - outBytesLeft);
                ba.resize(ba.size() * 2);
                outBytes = ba.data() + offset;
                outBytesLeft = static_cast<size_t>(ba.size() - offset);
            ...
            if (errno == EILSEQ) {
                ...
                state->saveChars(inBytes, static_cast<int>(inBytesLeft));
                remainingCount = static_cast<int>(inBytesLeft);
                break;
            }
    ...
    if (convState) {
        s = utf16Codec->toUnicode(ba.constData(),
                                  static_cast<int>(static_cast<unsigned long long>(ba.size()) - outBytesLeft),
                                  &state->internalState);
...
static bool setByteOrder(iconv_t cd)
{
    ...
    if (iconv(cd, inBytesPtr, &inBytesLeft, &outBytes, &outBytesLeft) == static_cast<size_t>(-1))
    {
        return false;
    }
#else
...
QByteArray QIconvCodec::convertFromUnicode(const QChar *uc, int len, ConverterState *convState) const
{
    ...
    inBytesLeft = static_cast<unsigned long>(len) * sizeof(QChar);

    QByteArray in;
    if (convState && convState->remainingChars) {
        // we have one surrogate char to be prepended
        in.resize(static_cast<vsizetype>(sizeof(QChar) + static_cast<unsigned long>(len)));
        inBytes = in.data();
    ...
    int invalidCount = 0;
    while (inBytesLeft != 0) {
        if (iconv(state->cd, inBytesPtr, &inBytesLeft, &outBytes, &outBytesLeft) == static_cast<size_t>(-1))
        {
    ...
    ba.resize(static_cast<vsizetype>(static_cast<unsigned long long>(ba.size()) - outBytesLeft));
...
iconv_t QIconvCodec::createIconv_t(const char *to, const char *from) const
{
    ...
    if (cd == reinterpret_cast<iconv_t>(-1))
    {
        ...
        if (cd == reinterpret_cast<iconv_t>(-1) && codeset && *codeset == '.')
        {
            ++codeset;
            cd = iconv_open(to ? to : codeset, from ? from : codeset);
        }
        ...
        // 3. ctype (maybe the locale is named "ISO-8859-1" or something)
        if (cd == reinterpret_cast<iconv_t>(-1) && ctype && *ctype != 0 && strcmp(ctype, "C") != 0)
            cd = iconv_open(to ? to : ctype, from ? from : ctype);


        // 4. locale (ditto)
        if (cd == reinterpret_cast<iconv_t>(-1) && lang && *lang != 0)
            cd = iconv_open(to ? to : lang, from ? from : lang);

        // 5. "@euro"
        if ((cd == reinterpret_cast<iconv_t>(-1) && ctype && strstr(ctype, "@euro")) || (lang && strstr(lang, "@euro")))
