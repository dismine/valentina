...
static int qt_Big5ToUnicode(const uchar *buf, uint *u)
{
    ...
    for(int i = 0; i < 4; i++) {
        ...
        uint b5 = static_cast<unsigned int>((buf[0] << 8) + buf[1]);
        while (start <= end) {
...
static int qt_Big5hkscsToUnicode(const uchar *s, uint *pwc)
{
    ...
    if ((c1 >= 0x81 && c1 <= 0xfe)) {
        ...
        if ((c2 >= 0x40 && c2 < 0x7f) || (c2 >= 0xa1 && c2 < 0xff)) {
            uint i = static_cast<uint>(157 * (c1 - 0x81) + (c2 - (c2 >= 0xa1 ? 0x62 : 0x40)));
            ushort wc = 0xfffd;
            if (i < 19782)
                wc = big5hkscs_to_ucs[i];
            if (wc != 0xfffd) {
                *pwc = static_cast<uint>(wc);
                return 2;
            }
        }
        return 0;
    } else if (c1 < 0x80) {
        *pwc = static_cast<uint>(c1);
        return 1;
    }
    return 0;
}
...
int qt_UnicodeToBig5hkscs (uint wc, uchar *r)
{
    ...
    if (summary) {
        ...
        if (used & (static_cast<ushort>(1) << i)) {
            ...
            used = static_cast<ushort>((used & 0x5555) + ((used & 0xaaaa) >> 1));
            used = static_cast<ushort>((used & 0x3333) + ((used & 0xcccc) >> 2));
            used = static_cast<ushort>((used & 0x0f0f) + ((used & 0xf0f0) >> 4));
            used = static_cast<ushort>((used & 0x00ff) + (used >> 8));
